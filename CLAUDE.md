...
**Файлы:** `frontend/src/components/GpuDetailModal.tsx`, `frontend/src/pages/Farm.tsx`

**Проблема:** Backend применяет `seasonMultiplier` и скидку провайдера **только к электричеству** (watt-based). Обслуживание (`igcMaintenancePerEpoch`) фиксировано и не дисконтируется. Frontend ошибочно применял оба множителя ко всему `igcCostPerDay` (электро + обслуживание), завышая экономию от провайдера в 2-4× для T3+.

**Исправление:** разделили `igcCostPerDay` на составляющие:
```typescript
const dailyElecBase = spec.wattBackend * 0.288; // 0.001 × 288
const dailyMaint    = spec.igcCostPerDay - dailyElecBase;
const elecDayCost   = (dailyElecBase * ocUvMult * electricityMult * providerMult)
                    + (dailyMaint    * ocUvMult);
```
OC/UV корректно масштабируют оба компонента; электромультипликаторы — только электро.

### Баг: GPU на 0% здоровье не ломалась автоматически

**Файл:** `backend/src/epoch/wearEngine.ts`

Если износ снижал health до 0% и `pFail` случайно не сработал — GPU оставалась `status='active'`, платила электричество, занимала слот, но майнила 0 хешрейта до ~3 дней.

**Исправление:** добавлена детерминированная поломка при `newHealth <= 0`:
```typescript
if (newHealth <= 0) {
  return { ..., broken: true }; // без RNG — деградированная карта всегда сломана
}
```

### Балансировка: wear ×2, break factor 864→96, Fan Lv1 250→100 IGC

**Файлы:** `backend/src/epoch/constants.ts`, `frontend/src/types.ts`, `frontend/src/pages/Guide.tsx`

**Цель:** игроки заходят раз в 1-2 дня, чувствуют необходимость проверять и чинить карты.

#### Скорость износа — удвоена

| Тир | Было | Стало | Дней до 50% (kTemp=1.0) |
|-----|------|-------|------------------------|
| T1 RX 580 | 0.003 | **0.006** | 58д → **29д** |
| T2 GTX 1660S | 0.004 | **0.008** | 43д → **22д** |
| T3 RTX 3070 | 0.005 | **0.010** | 35д → **17д** |
| T4 RTX 4090 | 0.007 | **0.014** | 25д → **12д** |
| T5 ASIC S19 | 0.010 | **0.020** | 17д → **9д** |
| T6 Quantum X1 | 0.003 | **0.006** | 58д → **29д** |

С OC (×2.5): T3 50% за **7 дней**, T5 за **4 дня**.

#### BREAKAGE_PROBABILITY_FACTOR: 864 → 96

Шанс случайной поломки за сутки:
- 50% здоровье: 3% → **31%**
- 30% здоровье: 10% → **64%**
- 10% здоровье: 33% → **89%**
...
Первый апгрейд вентилятора стал доступным для новичков T1-T2:
- T2 GTX 1660S Fan Lv1 ROI: 144д → **29 дней**
- T1 RX 580 Fan Lv1 ROI: 289д → **58 дней**

### Баг: Fan апгрейд не блокировался при достижении uptime cap (99%)

**Файл:** `frontend/src/components/GpuDetailModal.tsx`

T1 (base 90%) и T2 (base 88%) достигают 99% uptime cap до максимального уровня вентилятора. Игрок мог купить Fan Lv4 (4800 IGC) с нулевым эффектом.

**Исправление:** для каждого следующего уровня вентилятора вычисляется реальный прирост через `calcEffectiveUptime()` (учитывает UPS + провайдер + fan):
- `uptimeGain = nextUptime - currentUptime`
- Если `uptimeGain === 0`: кнопка заблокирована, показывается `"⚠️ Uptime на максимуме"`
- Инфо-карточка каждого уровня отображает реальный прирост или "uptime maxed"

### Uptime в pool-режиме — пояснение

Uptime — **постоянный множитель хешрейта**, не случайные эпохи оффлайн:
```typescript
farmHashrate += gpuH × (effectiveUptimePct / 100)
```
В пуле uptime пропорционально влияет на долю TON-наград и IGC-добычу. Электричество и обслуживание платятся независимо от uptime (для `status='active'` GPU).

### Итоговые ремонт-расходы/день с новым wear ×2 (kTemp=1.0, без апгрейдов)

| GPU | Ремонт/день | Чистые IGC/день |
|-----|------------|----------------|
| T1 RX 580 | 5.2 | **+23.6** |
| T2 GTX 1660S | 12.4 | **+30.8** |
| T3 RTX 3070 | 30.2 | **−30.2** |
| T4 RTX 4090 | 84.7 | **−113.5** |
| T5 ASIC S19 | 345.6 | **−547.2** |
| T6 Quantum X1 | 259.2 | **−259.2** |
