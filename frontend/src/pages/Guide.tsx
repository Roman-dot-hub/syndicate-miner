...
function FarmPrioritySection({ ru }: { ru: boolean }) {
  const items = ru ? [
    ...
    { n: 3, emoji: '📡', title: 'Провайдер как можно раньше', color: PU,
      desc: 'Lv1 всего 0.2 TON, но даёт −20% к IGC-расходу и +2% uptime. На 5 картах это 40+ IGC в день — окупается за несколько дней.' },
    { n: 4, emoji: '❄️', title: 'Серверная для снижения температуры', color: CY,
      desc: 'Снижает T_ambient для всей фермы. Ниже температура → медленнее износ → меньше тратишь на ремонт. Особенно важно при разгоне.' },
    ...
    { n: 3, emoji: '📡', title: 'ISP Contract early', color: PU,
      desc: 'Just 0.2 TON for Lv1, but gives −20% IGC cost and +2% uptime. With 5 GPUs that\'s 40+ IGC saved per day — pays off within days.' },
    { n: 4, emoji: '❄️', title: 'Server Room to lower temperature', color: CY,
      desc: 'Lowers T_ambient for the whole farm. Lower temp → slower wear → less spent on repair. Especially important when overclocking.' },
